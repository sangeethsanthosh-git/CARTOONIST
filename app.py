...
app.config['UPLOAD_FOLDER_IMAGES'] = 'static/uploaded_images'  # Add folder for images
app.config['CARTOONIZED_FOLDER'] = 'static/cartoonized_outputs'  # Update to handle both images and videos
        ...
        try:
            if flask.request.files.get('image'):
                ...
                if allowed_file(filename, 'image'):
                    ...
                    cartoonized_video_path = wb_cartoonizer.cartoonize_video(upload_path, cartoonized_video_path)  # Returns path with .avi extension

                    if not opts["run_local"]:
                        output_uri = upload_blob("cartoonized_videos", cartoonized_video_path, f"cartoonized_{filename}", content_type='video/avi')
